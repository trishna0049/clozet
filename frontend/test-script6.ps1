        ...
        <div className="rounded-[1.5rem] border border-white/70 bg-white p-4 shadow-soft">
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            <details className="group">
              <summary className="flex items-center gap-2 px-3 py-2 cursor-pointer text-sm font-medium text-cocoa">
                <span>Category</span>
                <span className="text-cocoa/60 text-xs">{selectedProductCategories.size ? ""${selectedProductCategories.size} selected"" : "All"}</span>
              </summary>
              <div className="mt-2 px-3 pb-4 space-y-3 text-sm text-cocoa/72">
...
"@
[System.IO.File]::WriteAllText("$pwd\test-parse-simple5.tsx", $beforeReturn + $test5Return + $afterReturn, [System.Text.Encoding]::UTF8)
Write-Host "Created test-parse-simple5.tsx"
